...
if (!APPS_SCRIPT_URL || APPS_SCRIPT_URL.includes('YOUR_SCRIPT_ID')) {
  console.error('[QTT] ERROR: Apps Script URL belum di-set di .env!');
}
if (!CLOUDINARY_CLOUD_NAME || CLOUDINARY_CLOUD_NAME === 'your_cloud_name') {
  console.error('[QTT] ERROR: Cloudinary config belum di-set di .env!');
}
...
async function uploadToCloudinary(file, username) {
  if (!CLOUDINARY_CLOUD_NAME || !CLOUDINARY_UPLOAD_PRESET) {
    throw new Error('Cloudinary config belum di-set di .env!');
  }
...
async function handleQttSubmit() {
  ...
  try {
    ...
    if (!cloudinaryUrl) {
      ...
      cloudinaryOriginalUrl = cloudResult.originalUrl;
    ...
    formData.append('videoUrl', cloudinaryUrl);
    formData.append('originalUrl', cloudinaryOriginalUrl || '');
    ...
    if (!APPS_SCRIPT_URL || APPS_SCRIPT_URL.includes('YOUR_SCRIPT_ID')) {
      throw new Error('Apps Script URL belum di-set di .env!');
    }
    ...
    const qttData = {
      ...
      gdUrl: result.videoUrl,
      cloudinaryOriginalUrl: cloudinaryOriginalUrl || '',
